...
def backward(A, B, q, seq):
    ...
    table = [[0 for j in range(B_col)] for j in range(int(seq[0])+1)]
    seq_len = int(seq[0])
    ...
    for i in range(1, seq_len+1):
        ...
        for x in range(A_row):
            ...
            for y in range(A_col):
                suma += float(last_row_table[x])*float(A_data[x]
                                                       [y])*float(B_data[y][int(seq[cur_seq])])
                # print("a {} b {} c {} res{}".format(
                #    last_row_table[y], A_data[x][y], B_data[y][int(seq[cur_seq])], suma))
            # print(suma)
            table[i][x] = suma
...
if __name__ == "__main__":
    ...
    backward(A, B, q, seq)    # Forward Algorithm
